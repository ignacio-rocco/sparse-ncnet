...
%         end
    ...
    linehandles(ii) = plot(errthr_dot, localized_rate_dot*100, method(ii).marker, 'color', method(ii).color, 'MarkerSize', method(ii).ms, 'LineWidth', 2.0, 'MarkerIndices',start_ind:2:length(errthr_dot));
...
svgname = fullfile(params.output.dir, sprintf('athr%.4f_%d.pdf', max_orierr, length(poserr_all)));
