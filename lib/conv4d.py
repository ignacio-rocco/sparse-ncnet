...
def conv4d(data,filters,bias=None,permute_filters=True,use_half=False,split_conv=False):
    ...
    if padding>0 and split_conv==False:
...
#        return conv4d(input, self.weight)
    
    def permute_filters(self):
        self.weight.data=self.weight.data.permute(2,0,1,3,4,5).contiguous()
        
    def unpermute_filters(self):
        self.weight.data=self.weight.data.permute(1,2,0,3,4,5).contiguous()
